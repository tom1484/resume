// Golden-set eval for the parse_jd stage (CLAUDE.md: required to pass
// before any prompt change lands). Re-parses 6 frozen real JDs live and
// asserts each expected term is recalled in mustHaves ∪ hardSkills.
//
// Usage: ANTHROPIC_API_KEY=... node eval/run-parse-eval.js
...
const fixtures = JSON.parse(readFileSync(new URL('./golden-jds.json', import.meta.url), 'utf8'));
const MIN_RECALL = 0.8;

let failed = false;
for (const fx of fixtures) {
  const { output } = await parseJd(fx);
  const extracted = [...output.mustHaves, ...output.hardSkills];
  const hits = fx.expectedTerms.filter((t) => extracted.some((e) => termMatches(t, e) || termMatches(e, t)));
