// Reads the labeled calibration CSV (label column: good | bad, unlabeled rows
// ignored) and reports precision/recall/F1 across thresholds plus the max-F1
// recommendation (gate per PROPOSALS §5 node 4).
// Usage: tsx eval/recommend-threshold.ts out/calibration.csv
...
export function evaluateThresholds(samples: Sample[]): ThresholdResult[] {
...
if (isMain) {
  ...
  if (!file) {
    console.error('usage: tsx eval/recommend-threshold.ts <calibration.csv>');
    process.exit(1);
  }
  ...
  for (const r of results) {
    console.log(
      `${r.threshold.toFixed(2)}  ${r.precision.toFixed(2)}  ${r.recall.toFixed(2)}  ${r.f1.toFixed(2)}`
    );
  }
  const best = results.reduce((a, b) => (b.f1 > a.f1 ? b : a));
  console.log(`\nrecommended scoreThreshold=${best.threshold} (F1=${best.f1.toFixed(2)})`);
}
