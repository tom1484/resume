...
export default function Publications({ title: sectionTitle, data, selectedTitles }) {
  ...
  return (
    <div className="flex flex-col items-end justify-center h-fit w-11/12">
      ...
      {data.map(({ title, authors, publication, time, link, content, tags }, idx) => (
        <React.Fragment key={idx}>
          <div className="flex items-stretch h-fit w-full">
            <h2 className="font-sans text-lg">{title}</h2>
          </div>
          <div className="flex items-stretch h-fit w-full">
            <h2 className="font-sans text-sm font-semibold text-neutral-500">
              {
                authors.map((author, idx) => (
                  author[0] === '!' ? 
                  <React.Fragment key={idx}>
                    <span className="font-bold text-black">{author.slice(1)}</span>{idx < authors.length - 1 && ', '}
                  </React.Fragment> :
                  <React.Fragment key={idx}>
