...
| P3.7 | Review app deployed on `nginx` net as `jobs-api:8080`, ready for NPM. **Blocked on Tom: enable NPM access list + point proxy host at jobs-api:8080 (steps in PREPARE.md)** — then phone-approval checkpoint | reachable on nginx net (200); public exposure + phone demo pending Tom's NPM action (PII must not go live without auth) | [~] |
